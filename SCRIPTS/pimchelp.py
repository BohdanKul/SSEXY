...
def getFileString(options,reduce=True):
    ...
    #variables that are not displayed in the filenames' structure
    #need to be taken a special care
    if options.V is not None:
       dataName = '%s|V-%15.8f'%(dataName,options.V)
    if options.R is not None:
       dataName = '%s|R-%15.8f' %(dataName,options.R) 

    #specifically tailored for reduce-one script:
    if reduce:
...
class PimcHelp:
    ...
    def __init__(self,dataName,canonical,baseDir=''):
        ...
        #parameters that are contained in pimc output filenames' structure
        #are stored in a string 
        self.dataName = dataNameList.pop(0) 
        #other are stored in a dictionnary
        self.hoptions = {}
        for hoption in dataNameList:
            self.hoptions[hoption.split('-')[0]] = hoption.split('-')[1]
        if canonical:
            self.prefix='ce'
        else:
            self.prefix='gce'
    ...
    def getParameterMap(self,logName): 
        ...
        # Add an element to the parameter map for the linear dimension (r) of
        # the cylindrical container
        paramsMap['Cylindrical Container Radius'] = '%6.5f' %(float(paramsMap['Container Dimensions'].split('x')[-2].strip())/2)

        # Add an element to the parameter map for the number of update slices (flag -M)
        paramsMap['Update Slices (Mbar)'] = paramsMap['Update Slices (Mbar)'].split('x')[-1]
       
        commandLine = logLines[2]
        commands    = commandLine.split()
        paramsMap['Seed number'] = int(commands[commands.index('-p')+1])
        print paramsMap['Seed number']
        return paramsMap
    ...
    # -----------------------------------------------------------------------------
    def ApplyImplicitParameters(self): 
        '''Once getSimulationParameters is executed, this method deletes those PIMCid's
           that dont satisfy implicit parameters: R,V '''

        todel = []
        if self.hoptions:
           for pimcID in self.params.keys():
               if (self.hoptions.has_key('R')):
                  if (len(self.params[pimcID]) == 0):
                     todel.append(pimcID)
                     continue
                  if (self.params[pimcID]['Container Type'] == 'Cylinder'):
                     radius = float(self.params[pimcID]['Container Dimensions'].split('x')[-2].strip())/2
                     if float(self.hoptions['R']) != radius:
                        todel.append(pimcID)
                        continue 
                  else:
                        todel.append(pimcID)  
                        continue
               if self.hoptions.has_key('V'):  
                  volume = float(self.params[pimcID]['Container Volume'])
                  if float(self.hoptions['V']) != volume:
                     todel.append(pimcID)           
       
        for delitem in todel:
            self.id.remove(delitem)
            self.params.pop(delitem)         
    ...
    def getFileList(self,type,idList=None):
        ...
        # Otherwise we just go through and get the ID's we need
            ...
            for id in idList: 
                lsCommand = 'ls -1 %s%s-%s-*%s.dat' % (self.baseDir,self.prefix,type,id)
                print id
                print os.popen(lsCommand).read().split('\n')
...
class ScalarReduce:
    ...
    def __init__(self,fileNames,varLabel=None):
        ...
        #print "reduce Label ", self.reduceLabel
        ...
        self.estimator_ = np.zeros([len(fileNames), 
